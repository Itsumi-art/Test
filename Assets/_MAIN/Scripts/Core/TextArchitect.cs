...
public class TextArchitect 
{
    ...
    private int preTextLength = 0;  //! Linha de aviso Unity, resolver depois

    public string fullTargetText => preText + targetText;

    public enum BuildMethod { instant, typewriter, fade}
    //3 Methods difererentes para construir coisas
    ...
    public Coroutine Build(string text) {
        //Build Text to what is already in the text architect
        preText = "";
        targetText = text;
    ...
    private void Prepare_Instant() {
        tmpro.color = tmpro.color;//redefinir cor, verificar se está visivel
        tmpro.text = fullTargetText;//verificar que tenho o texto que estou a atribuir
        tmpro.ForceMeshUpdate(); //Todas as alterações serão aplicadas
        tmpro.maxVisibleCharacters = tmpro.textInfo.characterCount; //Text.Info vai ser regenerado quando forçar atualização/certificar que todos os characters estou visiveis na tela
    }

    private void Prepare_Typewriter() {
        // Garatir que o texto está numa boa ordem
    ...
    private IEnumerator Build_Typewriter() {
        //Velocidade do efeito máquina de escrever/Typewriter
